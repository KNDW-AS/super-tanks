...
| **R-08** | An agent mass-reads or writes memory paths it shouldn't | High | Medium | `SecureMemoryStore` enforces tripwire + RBAC + audit on every op (`core/memory/secure_store.py`); `is_path_accessible` denies unknown paths fail-closed (`core/memory/access_control.py`); tripwire honeypots at 5 paths trigger −100 trust + forced LOCKDOWN | **Low.** Closed 2026-07-25: every agent-facing call site now goes through `SecureMemoryStore` — shadow-proposal diff reads, the approval merge (which previously wrote to an arbitrary path with no RBAC and no audit entry), and hybrid-search L0 reads (RBAC was enforced inline, but the reads left no audit trail). A denied approval now leaves the proposal pending instead of recording a merge that never happened. Remaining raw usage is the tripwire machinery itself — `bootstrap`, `tripwires`, `proactive_monitor` deploy and verify the honeypots, and cannot go through the layer that alarms on them — plus `hierarchical_search()` candidate generation, whose only caller RBAC-filters before returning. Each exemption is listed with a written reason in `tests/test_memory/test_raw_store_allowlist.py`, which fails on any new unlisted raw import and on stale exemptions. |
