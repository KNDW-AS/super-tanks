...
| **R-02** | Indirect prompt injection via memory/web/file content re-fed to LLM | Critical | Medium | ZEF runs at message ingress only; tripwires detect read attempts on honeypot paths | **High** — tool outputs are not re-scanned; this is a known follow-up |
...
| **R-06** | Attacker forges A2A messages claiming `sender="william"` to escalate via inter-agent channel | High | Low | `core.security.agent_identity.sign_a2a_message` / `verify_a2a_message` exists, A2A in HIGH_RISK_CHANNELS for ZEF | **High** in this repo — production A2A wiring is out-of-tree; `verify_a2a_message` has no caller in `core/a2a/`. Open follow-up. |
...
| **R-12** | Audit log tampered to hide an action | High | Medium | WAL-mode SQLite; per-call connection (no shared cursor races); append-only by convention | **High** — no chained-hash / MAC; an attacker with FS write to `data/memory_audit.db` can rewrite history. Open follow-up: tamper-evident audit. |
...
| **R-20** | A2A message with forged sender field reaches the recipient | High | Medium | `A2AMessage.signature` field with HMAC; `verify_a2a_message` available | **High** — verification is not invoked anywhere in `core/a2a/` (only `escalation_rules.py` lives in this repo); see R-06 |
| **R-21** | Tool dispatch happens but no record exists tying it to a session / approval / trust event | Medium | High | `gateway.dispatch_tool` only `logger.debug`s; the audit DB is for memory ops only | **High** — open follow-up P1: gateway-level dispatch audit with correlation_id propagated to memory_audit, trust_events, approval_requests |
...
|:---:|---|---|:---:|
| 1 | R-21 | Gateway-level dispatch audit with `correlation_id` across memory_audit / trust_events / approval_requests | Medium |
| 2 | R-12 | Tamper-evident audit log (chained HMAC over rows) | Medium |
| 3 | R-02 | Re-scan tool outputs (`web_browse`, `file_read`, `memory_read`) for indirect prompt injection before LLM re-injection | Medium |
| 4 | R-05 | Sign integrity manifests (`soul_integrity.json`, `DIQ_CHECKSUMS.json`) with offline / TPM key | Large |
| 5 | R-06, R-20 | Wire `verify_a2a_message` into the production A2A receive path (lives outside this repo today) | Small (in this repo) / Medium (out-of-tree) |
| 6 | R-04 | Runtime sandbox (nsjail/firejail/docker-uid) wrapping approved code execution; AST is preventative-only today | Large |
| 7 | R-22 | Adversarial fuzzing harness for AST scanner + ZEF filter; track FPR/FNR | Medium |
| 8 | R-14 | Gate `trust_score.record_event` behind a process-internal capability so only the audit subsystem can mutate trust | Medium |
| 9 | R-08 | Audit + deprecate every direct caller of `HierarchicalMemoryStore`; route through `SecureMemoryStore` | Medium |
| 10 | R-15 | Cooperative cancellation of in-flight tool calls when LOCKDOWN engages | Large |
