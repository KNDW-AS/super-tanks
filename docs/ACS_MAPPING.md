...
| **2. LLM** | ZEF Firewall (1) as a pre-model filter; Provider Trust Tier (11) stripping the prompt for the target provider's trust tier; Provider Failover GO-Gate (12) blocking silent switches to less-trusted providers; Circuit Breaker (7) modes (LOCKDOWN / Night) and trust score constraining when/whether the model is invoked | `core/security/zef_llm_classifier.py`; `core/security/provider_trust.py`, `core/security/provider_failover.py`, `core/council/council.py`; `core/security/super_tanks_mode.py`, `core/security/trust_score.py` |
...
| **4. Tool execution** | Allowlists (4, default-deny) + GO-Gate (5, human-in-the-loop) + Sandbox (6, static AST scan — preventative-only, see R-04) + Circuit Breaker (7) + Tool Zone Isolation (8) + MCP Security Manager (9) + allowed_agents (10) | Dispatch gate `dispatch_tool` / `_dispatch_inner` in `core/gateway.py` calling `is_tool_allowed` (`core/security/tool_allowlists.py`, fails closed); GO-Gate daemon `core/go_gate_approval_daemon.py`; AST scanner `core/zeph_quarantine_ast.py`; skill-level isolation via `allowed_agents` in `core/diq/diq_skills.py` |
