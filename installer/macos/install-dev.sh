...
set -euo pipefail
OLLAMA=0; SKIP=0; DIR=""
while [ $# -gt 0 ]; do case "$1" in --ollama) OLLAMA=1;; --skip-tests) SKIP=1;; --dir) DIR="${2:-}"; [ -n "$DIR" ] || { echo "--dir needs a path"; exit 2; }; shift;; *) echo "unknown arg $1"; exit 2;; esac; shift; done
...
if ! xcode-select -p >/dev/null 2>&1; then
  warn "Xcode Command Line Tools missing. Run: xcode-select --install   (then rerun this script)"; exit 1
fi
...
for c in python3.12 python3; do
  if command -v "$c" >/dev/null 2>&1 && "$c" -c 'import sys; sys.exit(0 if (3,10)<=sys.version_info[:2]<=(3,12) else 1)' 2>/dev/null; then PY="$c"; break; fi
done
...
.venv/bin/python -m pip install -e ".[dev]" --quiet || { warn "pip install failed — see above. Usually wrong Python (need 3.10–3.12) or no internet."; exit 1; }
ok "Package installed"
say "Environment check"
.venv/bin/python -m supertanks doctor || true

# 5. Tests
if [ "$SKIP" -eq 0 ]; then
  say "Running unit tests (expect ~1,300 passed)"
  if .venv/bin/python -m pytest -q --no-cov -p no:cacheprovider; then ok "All tests passed"; else warn "Some tests FAILED. Do not continue on a red suite: copy the output and send it to William (billyxp@gmail.com)."; fi
...
if [ "$OLLAMA" -eq 1 ]; then
  ...
  ollama pull llama3.2:3b || warn "ollama pull llama3.2:3b failed (is ollama serve running?)"
  ollama pull nomic-embed-text || warn "ollama pull nomic-embed-text failed"
  ok "Ollama ready on http://localhost:11434"
fi

echo; echo "  Done. Next steps:"
echo "    cd $TARGET && source .venv/bin/activate"
echo "    python -m supertanks doctor   # environment check"
echo "    python -m supertanks demo     # GO-Gate demo"
echo "    python -m supertanks test     # tests"
